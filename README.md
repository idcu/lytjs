...
| `@lytjs/renderer` | 渲染器主入口（DOM/SSR/Vapor） | 5.00 KB |
...
| 包名 | 说明 |
|------|------|
| `@lytjs/router` | 内置路由（History/Hash/守卫/动态路由） |
| `@lytjs/store` | 内置状态管理（Pinia 风格） |
| `@lytjs/components` | UI 组件库（38+ 组件/主题系统） |
| `@lytjs/cli` | 命令行工具（create/dev/build） |
| `@lytjs/devtools` | 浏览器调试面板 |
