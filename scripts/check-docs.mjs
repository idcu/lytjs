...
async function main() {
  ...
  for (const r of allResults) {
    ...
    if (r.hasReadme) {
      console.log(`   README: ${r.readmeChinese ? '中文' : '英文'} ${r.versionInReadme ? `(v${r.versionInReadme})` : ''}`);
    } else {
    ...
    if (r.hasChangelog) {
      console.log(`   CHANGELOG: ${r.changelogChinese ? '中文' : '英文'} ${r.versionInChangelog ? `(v${r.versionInChangelog})` : ''}`);
    } else {
