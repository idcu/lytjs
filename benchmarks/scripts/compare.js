...
function printComparison(results) {
  ...
  for (const [suiteName, benchmarks] of Object.entries(grouped)) {
    ...
    for (const b of benchmarks) {
      ...
      const changeStr = b.change.mean !== null 
...
function main() {
  ...
  if (!baseline) {
    console.warn(`${colors.yellow}Warning: No baseline found. Saving current as baseline.${colors.reset}`);
  ...
  const significantRegressions = results.filter(r => 
    r.baseline && r.change.mean > 10
  );
  
  if (significantRegressions.length > 0) {
    console.log(`${colors.red}ERROR: ${significantRegressions.length} significant regressions detected (>10%)${colors.reset}`);
    process.exit(1);
  }
}
