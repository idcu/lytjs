...
- **零耦合**：各包可独立运行，不互相依赖（monorepo 解耦设计）
- **最小化**：每个包只实现核心功能，保持轻量
...
使用包名作为 scope：`reactivity`、`compiler`、`vdom`、`renderer`、`component`、`router`、`store`、`core`、`cli`、`devtools`。
...
Lyt.js v1.0 引入了 Composition API，在贡献相关代码时请遵循以下规范。
...
- `@lytjs/plugin-i18n`：国际化插件，支持多语言切换和复数形式
