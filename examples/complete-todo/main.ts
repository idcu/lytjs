...
function TodoApp() {
  ...
  // 渲染主题切换按钮
  const renderThemeButton = () => h('div', { class: 'controls' }, [
    h('button', { 
  ...
  // 渲染头部
  const renderHeader = () => h('div', { class: 'header' }, [
    h('h1', {}, '📋 Lyt.js 待办应用'),
    h('p', {}, '一个功能完整的待办事项管理应用'),
    renderThemeButton()
  ]);
  ...
  // 渲染输入区域
  const renderInputSection = () => h('div', { class: 'todo-input-section' }, [
  ...
  // 渲染单个待办项
  const renderTodoItem = (todo: Todo) => h('div', {
  ...
  // 渲染待办列表
  const renderTodoList = () => h('div', { class: 'todo-list' }, [
    ...filteredTodos.value.map(renderTodoItem),
    todos.value.length > 0 ? h('div', { class: 'footer' }, [
      h('span', {}, `${activeCount.value} 项未完成`),
      hasCompleted.value ? h('button', {
