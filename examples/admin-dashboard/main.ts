...
const initialUsers: User[] = [
  ...
  { id: 2, name: '李四', email: 'lisi@example.com', role: 'editor', status: 'active', createdAt: '2024-02-20' },
  { id: 3, name: '王五', email: 'wangwu@example.com', role: 'user', status: 'pending', createdAt: '2024-03-10' },
  { id: 4, name: '赵六', email: 'zhaoliu@example.com', role: 'user', status: 'inactive', createdAt: '2024-01-25' },
];

const initialProducts: Product[] = [
  { id: 1, name: 'LytJS 学习包', category: '教程', price: 99, stock: 150, description: '完整的 LytJS 学习资料' },
  { id: 2, name: '开发者工具套装', category: '工具', price: 199, stock: 80, description: '提升开发效率的工具' },
  { id: 3, name: '企业版许可', category: '许可', price: 999, stock: 50, description: '企业级使用许可' },
  { id: 4, name: '技术咨询服务', category: '服务', price: 500, stock: 10, description: '专业技术咨询服务' },
];
...
const App = defineComponent({
  ...
  setup() {
    ...
    // 渲染函数
    return () => h('div', { class: `dashboard ${theme.value}` }, [
      // 侧边栏
      h('aside', { 
      ...
      h('main', { class: 'main-content' }, [
        ...
        h('div', { class: 'content' }, [
          // 仪表盘
          currentTab.value === 'dashboard' ? h('div', { class: 'dashboard-view' }, [
            h('h2', '📊 仪表盘'),
            h('div', { class: 'stats-grid' }, [
              ...
              h('div', { class: 'stat-card' }, [
                h('div', { class: 'stat-value' }, `¥${stats.value.totalRevenue.toLocaleString()}`),
            ...
            h('div', { class: 'dashboard-section' }, [
              ...
              stats.value.lowStock > 0 ? 
                h('div', { class: 'warning-box' }, `⚠️ 有 ${stats.value.lowStock} 个产品库存不足 50`) :
                h('div', { class: 'success-box' }, '✅ 所有产品库存充足')
            ]),
            
            h('div', { class: 'dashboard-section' }, [
              h('h3', '📋 待处理订单'),
              h('ul', { class: 'orders-preview' }, 
                orders.value.filter(o => o.status === 'pending').map(order =>
          ...
          // 用户管理
          currentTab.value === 'users' ? h('div', { class: 'users-view' }, [
              ...
              h('table', [
                ...
                h('tbody', 
                  filteredUsers.value.map(user =>
                    h('tr', { key: user.id, class: { 'selected': selectedUser.value?.id === user.id } }, [
                      h('td', String(user.id)),
                      h('td', user.name),
                      h('td', user.email),
            ...
            selectedUser.value ? h('div', { class: 'user-details' }, [
              h('div', { class: 'modal-overlay' }, [
                h('div', { class: 'modal' }, [
                  ...
                  h('div', { class: 'detail-item' }, [
                    ...
                    h('span', { class: `role-badge ${selectedUser.value.role}` }, selectedUser.value.role)
                  ]),
                  h('div', { class: 'detail-item' }, [
                    h('span', { class: 'detail-label' }, '状态:'),
                    h('span', { class: `status-badge ${selectedUser.value.status}` }, selectedUser.value.status)
                  ]),
                  h('div', { class: 'detail-item' }, [
          ...
          // 产品管理
          currentTab.value === 'products' ? h('div', { class: 'products-view' }, [
            h('div', { class: 'view-header' }, [
              h('h2', '📦 产品管理'),
              h('button', { 
              ...
              products.value.map(product =>
                h('div', { class: 'product-card', key: product.id }, [
                  ...
                  h('div', { class: 'product-stock' }, [
                    h('span', { class: product.stock < 50 ? 'stock-low' : 'stock-normal' }, 
                      `库存: ${product.stock}`
                    )
                  ]),
            ...
            productFormOpen.value && editingProduct.value ? h('div', { class: 'product-form-modal' }, [
              h('div', { class: 'modal-overlay' }, [
                h('div', { class: 'modal' }, [
                    ...
                    h('input', { 
                      ...
                      onInput: (e: any) => {
                        if (editingProduct.value) {
                          editingProduct.value = { ...editingProduct.value, name: e.target.value };
                        }
                      }
                    })
                  ]),
                  h('div', { class: 'form-group' }, [
                    ...
                    h('input', { 
                      ...
                      onInput: (e: any) => {
                        if (editingProduct.value) {
                          editingProduct.value = { ...editingProduct.value, category: e.target.value };
                        }
                      }
                    })
                  ]),
                  h('div', { class: 'form-group' }, [
                    ...
                    h('input', { 
                      ...
                      onInput: (e: any) => {
                        if (editingProduct.value) {
                          editingProduct.value = { ...editingProduct.value, price: Number(e.target.value) };
                        }
                      }
                    })
                  ]),
                  h('div', { class: 'form-group' }, [
                    ...
                    h('input', { 
                      ...
                      onInput: (e: any) => {
                        if (editingProduct.value) {
                          editingProduct.value = { ...editingProduct.value, stock: Number(e.target.value) };
                        }
                      }
                    })
                  ]),
                  h('div', { class: 'form-group' }, [
                    ...
                    h('textarea', { 
                      ...
                      onInput: (e: any) => {
                        if (editingProduct.value) {
                          editingProduct.value = { ...editingProduct.value, description: e.target.value };
                        }
                      }
                    })
                  ]),
          ...
          // 订单管理
          currentTab.value === 'orders' ? h('div', { class: 'orders-view' }, [
            h('h2', '🛒 订单管理'),
            h('div', { class: 'table-container' }, [
              h('table', [
                h('thead', [
                ...
                h('tbody', 
                  orders.value.map(order =>
                    h('tr', { key: order.id }, [
                      ...
                      h('td', order.date),
                      h('td', { class: 'actions' }, 
                        order.status === 'pending' ? h('button', { 
                          class: 'btn-small btn-primary',
                          onClick: () => updateOrderStatus(order.id, 'processing')
                        }, '处理') :
                        order.status === 'processing' ? h('button', { 
                          class: 'btn-small',
                          onClick: () => updateOrderStatus(order.id, 'shipped')
                        }, '发货') :
                        order.status === 'shipped' ? h('button', { 
