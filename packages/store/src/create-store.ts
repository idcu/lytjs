...
export function createStore<S extends Record<string, any> = Record<string, any>>(
  ...
  /**
   * 创建一个快照（浅拷贝原始对象），用于比较变化
   */
  function createSnapshot(state: any): Record<string, any> {
   ...
   * 比较两个快照，找出差异并通知订阅者
   */
  function diffAndNotify(oldSnapshot: Record<string, any>, newSnapshot: Record<string, any>): void {
    const allKeys = new Set([...Object.keys(oldSnapshot), ...Object.keys(newSnapshot)]);

    for (const key of allKeys) {
      const hadOld = key in oldSnapshot;
      const hadNew = key in newSnapshot;
      const oldValue = oldSnapshot[key];
      const newValue = newSnapshot[key];

      if (!hadOld && hadNew) {
        // 新增属性
        notifySubscribers({ storeId: id, type: 'add', key, newValue });
      } else if (hadOld && !hadNew) {
        // 删除属性
        notifySubscribers({ storeId: id, type: 'delete', key, oldValue });
      } else if (!Object.is(oldValue, newValue)) {
        // 修改属性
        notifySubscribers({ storeId: id, type: 'set', key, newValue, oldValue });
      }
    }
  }
  ...
  function ensureWatch(): void {
    ...
    activeWatch = watch(
      ...
      () => {
        ...
        diffAndNotify(prevSnapshot, newSnapshot);
  ...
  function stopWatchIfNoSubscribers(): void {
    if (subscriptions.length === 0 && activeWatch) {
      activeWatch();
  ...
  const storeApi: StoreApi<S> = {
    ...
    $patch(partialOrFn: Partial<S> | ((state: S) => void)): void {
      ...
      if (typeof partialOrFn === 'function') {
        ...
        // 对象合并：将 partial 中的属性合并到当前状态中
        for (const key of Object.keys(partialOrFn)) {
          (reactiveState as any)[key] = (partialOrFn as any)[key];
        }
      }
    },
