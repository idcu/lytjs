...
//
// ⚠️ 状态说明（2026-09 审计）：本模块**未接入编译管线** ——
//   它只通过 `optimizations/index.ts` 导出（编译时静态分析的分析 API），
//   但 `packages/compiler/src` 内**没有任何编译路径引用它**（实测外部引用 0 次，亦无测试）。
//   如需对外提供该能力，请显式从 index.ts 导出并在文档中说明；否则应视为未完成功能。
//
import type {
  RootNode,
  TemplateChildNode,
...
function checkKeyAttribute(node: ElementNode, context: AnalysisContext): void {
  const hasVFor = node.props.some((p) => p.type === NodeTypes.DIRECTIVE && p.name === 'for');
...
function checkEventNaming(node: ElementNode, context: AnalysisContext): void {
  const events = node.props.filter(
...
function checkStyleAttribute(node: ElementNode, context: AnalysisContext): void {
  const styleAttr = node.props.find(
...
export function formatStaticAnalysisResult(result: StaticAnalysisResult): string {
  ...
  if (
    ...
    result.performanceTips.length === 0
  ) {
    output.push('\n✅ 未发现问题！');
  }
