...
import type { NodeTransform, TransformContext, ElementNode, DirectiveNode } from '../types';
 ...
 */
export const transformScoped: NodeTransform = (node, context) => {
 ...
 */
function processVDeep(element: ElementNode, scopeId: string, _context: TransformContext): void {
  for (let i = 0; i < element.children.length; i++) {
    ...
    if (child.type === NodeTypes.ELEMENT) {
      ...
      const vDeepDir = childElement.props.find(
        (p): p is DirectiveNode => p.type === NodeTypes.DIRECTIVE && p.name === 'deep',
      );
