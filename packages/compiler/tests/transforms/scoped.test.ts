...
describe('transformScoped', () => {
  ...
  describe('multiple elements', () => {
    it('should add scopeId to each element independently', () => {
      ...
      expect(div.props.some((p) => p.type === NodeTypes.ATTRIBUTE && p.name === scopeId)).toBe(true);
      expect(span.props.some((p) => p.type === NodeTypes.ATTRIBUTE && p.name === scopeId)).toBe(true);
