...
describe('Parser 边界条件', () => {
  describe('空输入与空白输入', () => {
    ...
    it('多个连续空白字符应被压缩', () => {
      const ast = parse('   a   b   ');
      // 非空白文本节点之间的空白应被压缩
      const textNodes = ast.children.filter(
        (n) => n.type === NodeTypes.TEXT,
      ) as TextNode[];
      // 至少有一个包含 "a" 和 "b" 的文本节点
...
describe('指令解析边界条件', () => {
  ...
  describe('v-slot 嵌套', () => {
    ...
    it('作用域插槽 v-slot="props"', () => {
      const ast = parse('<template v-slot="slotProps"><span>{{ slotProps.item }}</span></template>');
      const el = ast.children[0] as ElementNode;
      const dir = el.props[0] as DirectiveNode;
      expect(dir.name).toBe('slot');
  ...
  it('深层嵌套的 v-for', () => {
    const ast = parse(
      '<div v-for="row in rows">' +
        '<span v-for="col in row.cols">{{ col }}</span>' +
      '</div>',
    );
    expect(ast.children.length).toBeGreaterThan(0);
  });
