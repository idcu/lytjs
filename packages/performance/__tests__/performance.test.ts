...
function createMockFn<T extends (...args: any[]) => any>(impl?: T): T & { mock: { calls: any[][]; results: any[] } } {
  const mock = ((...args: any[]) => {
...
function useFakeTimers() {
  ...
  const timers: Array<{ fn: () => void; delay: number; interval?: boolean; id?: any }> = [];
  ...
  (globalThis as any).setInterval = ((fn: () => void, delay: number) => {
    const id = timers.length + 1;
    timers.push({ fn, delay, interval: true, id });
    return id;
  }) as any;
  (globalThis as any).clearInterval = (() => {}) as any;
  return {
    advanceTimersByTime(ms: number) {
      // Execute all timers with delay <= ms
...
function setupMocks() {
  ...
  const mockPerformance = {
    mark: createMockFn(),
    measure: createMockFn(),
    clearMarks: createMockFn(),
    clearMeasures: createMockFn(),
    getEntriesByName: createMockFn(() => []),
    getEntriesByType: createMockFn((type: string) => {
      if (type === 'navigation') {
  ...
  (globalThis as any).document = {
    ...
    addEventListener: createMockFn(),
    removeEventListener: createMockFn(),
    querySelectorAll: createMockFn(() => ({ length: 42 })),
  };
  ...
  (globalThis as any).navigator = {
    sendBeacon: createMockFn(() => true),
  };

  // Mock fetch
  (globalThis as any).fetch = createMockFn(() => Promise.resolve());
...
describe('Web Vitals', () => {
  ...
  it('should register and trigger vital callbacks', async () => {
    ...
    expect(lastCall[0].name).toBe('FCP');
    expect(lastCall[0].value).toBe(1200);
    expect(lastCall[0].rating).toBe('good');
  ...
  it('should support wildcard callback', async () => {
    ...
    const lastCall = callback.mock.calls[callback.mock.calls.length - 1];
    expect(typeof lastCall[0].name).toBe('string');
    expect(lastCall[0].name).toBe('LCP');
    expect(lastCall[0].value).toBe(2800);
    expect(lastCall[0].rating).toBe('needs-improvement');
  ...
  it('should handle CLS accumulation', async () => {
    ...
    expect(Math.abs(cls!.value - 0.13)).toBeLessThan(0.01);
...
describe('Memory Tracking', () => {
  ...
  it('should respect maxSamples config', async () => {
    ...
    // 产生 10 个采样
    fakeTimers.advanceTimersByTime(1100);
...
describe('Reporter', () => {
  ...
  it('console reporter should call console methods', async () => {
    ...
    expect((console as any).group.mock.calls.length).toBeGreaterThanOrEqual(1);
    expect((console as any).log.mock.calls.length).toBeGreaterThanOrEqual(1);
    expect((console as any).groupEnd.mock.calls.length).toBeGreaterThanOrEqual(1);
  });
  ...
  it('fetch reporter should use sendBeacon', async () => {
    ...
    expect((navigator as any).sendBeacon.mock.calls.length).toBeGreaterThanOrEqual(1);
  ...
  it('fetch reporter should respect sample rate', async () => {
    ...
    expect((navigator as any).sendBeacon.mock.calls.length).toBe(0);
