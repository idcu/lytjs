...
import { allChildrenHaveKeys, noChildrenHaveKeys, analyzeDiff } from '../src/diff';
...
describe('VNode 创建边界', () => {
  ...
  describe('mergeProps 多个 props 合并', () => {
    it('三个 props 对象应正确合并', () => {
      const result = mergeProps({ id: 'a' }, { class: 'b' }, { style: { color: 'red' } });
  ...
  describe('mergeProps class/style 合并', () => {
    it('多个 class 应拼接', () => {
      const result = mergeProps({ class: 'a' }, { class: 'b' }, { class: 'c' });
...
describe('Fragment 边界', () => {
  ...
  it('getFragmentChildCount 应正确计算扁平化后的子节点数', () => {
    const innerFrag = createFragment([createVNode('span'), createVNode('span')]);
    const outerFrag = createFragment([createVNode('div'), innerFrag]);
...
describe('Diff 分析边界', () => {
  ...
  describe('allChildrenHaveKeys / noChildrenHaveKeys', () => {
    it('allChildrenHaveKeys 全部有 key 应返回 true', () => {
      const children = [createVNode('div', { key: 'a' }), createVNode('span', { key: 'b' })];
      expect(allChildrenHaveKeys(children)).toBe(true);
    });

    it('allChildrenHaveKeys 部分无 key 应返回 false', () => {
      const children = [createVNode('div', { key: 'a' }), createVNode('span')];
