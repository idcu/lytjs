...
> **框架无关性说明**：本插件为框架生态包，零第三方依赖，仅构建于 LytJS 核心之上（依赖 `@lytjs/core`、`@lytjs/reactivity` 与 `@lytjs/common-is`，均实际使用，无需清理）。按 v6.12 路线图规划，所有 @lytjs/plugin-\* 插件将统一迁出为独立仓库 lytjs-plugins，保持 API 兼容并提供迁移指南。
