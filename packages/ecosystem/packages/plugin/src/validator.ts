// packages/ecosystem/packages/plugin/src/validator.ts
// 通用插件系统 - 插件验证器（零框架依赖）
...
/**
 * 插件内部结构类型
 * @description 验证器仅读取插件字段，不关心宿主上下文（TContext），
 * 使用 unknown 字段放宽签名差异，便于接受任意上下文的具体插件实例。
 */
 ...
 * 验证规则函数类型
 */
type ValidationRule = (plugin: PluginLike, context: ValidationContext) => ValidationIssue[];
...
export class PluginValidator {
   ...
   * @returns 验证报告
   */
  validate<TOptions = unknown, TContext = unknown>(
    plugin: EnhancedPlugin<TContext, TOptions> | Plugin<TContext, TOptions>,
  ): ValidationReport {
    const like: PluginLike = plugin as PluginLike;
    const pluginName = this.getPluginName(like) ?? 'anonymous';
    ...
    for (const rule of this.rules) {
      try {
        const ruleIssues = rule(like, context);
   ...
   * @returns 验证报告列表
   */
  validateAll<TOptions = unknown, TContext = unknown>(
    plugins: (EnhancedPlugin<TContext, TOptions> | Plugin<TContext, TOptions>)[],
  ): ValidationReport[] {
   ...
   */
  private validateStructure(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
   ...
   */
  private validateName(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
   ...
   */
  private validateVersion(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
   ...
   */
  private validateDependencies(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    const dependencies = plugin.dependencies as PluginDependency[] | undefined;
    const optionalDependencies = plugin.optionalDependencies as PluginDependency[] | undefined;
    ...
    if (dependencies) {
      if (!Array.isArray(dependencies)) {
        ...
        issues.push(...this.validateDependencyArray(dependencies, 'dependencies', false));
      }
    }
   ...
   */
  private validateConflicts(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    const conflicts = plugin.conflicts as string[] | undefined;
    ...
    if (conflicts) {
      if (!Array.isArray(conflicts)) {
        ...
        // 检查是否与自身冲突
        if (plugin.name && conflicts.includes(plugin.name as string)) {
          issues.push({
            level: 'error',
            rule: 'validate-conflicts',
   ...
   */
  private validatePeerRequirements(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
    ...
    if (plugin.peerRequirements) {
      ...
      const { lytjs, node } = plugin.peerRequirements as { lytjs?: string; node?: string };
   ...
   */
  private validateLifecycleHooks(plugin: PluginLike): ValidationIssue[] {
    const issues: ValidationIssue[] = [];
   ...
   * 获取插件名称
   */
  private getPluginName(plugin: PluginLike): string | undefined {
    if (this.isEnhancedPlugin(plugin)) {
   ...
   */
  private isEnhancedPlugin(plugin: PluginLike): boolean {
    return (
      plugin != null &&
