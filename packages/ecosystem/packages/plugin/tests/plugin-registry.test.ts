...
import type { EnhancedPlugin, Plugin, PluginLifecycleEvent } from '../src/index';
...
describe('PluginRegistry', () => {
  ...
  describe('事件系统', () => {
    ...
    it('应触发所有生命周期事件', () => {
      ...
      registry.on('before:register', handler);
      registry.on('after:register', handler);
      registry.on('before:unregister', handler);
      registry.on('after:unregister', handler);
