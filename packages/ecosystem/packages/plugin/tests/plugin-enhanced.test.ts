...
import { PluginRegistry, PluginValidator } from '../src/index';
import type { EnhancedPlugin, Plugin } from '../src/index';

/**
 * EnhancedPlugin 集成测试
 *
 * 验证 PluginRegistry + PluginValidator 的协作逻辑，属于 @lytjs/plugin 独立包的
 * 零框架依赖测试，不依赖 core 的 createApp 集成。
 */
describe('EnhancedPlugin 集成测试', () => {
  describe('Registry + Validator 协作', () => {
    ...
    it('事件系统应跟踪完整生命周期', () => {
      ...
      registry.on('before:register', (event, data) => events.push({ event, data }));
      registry.on('after:register', (event, data) => events.push({ event, data }));
  ...
  describe('自定义验证规则', () => {
    it('应支持添加自定义验证规则', () => {
      ...
      validator.addRule((plugin) => {
        if (typeof plugin.name === 'string' && !plugin.name.startsWith('@mycompany/')) {
          return [
            {
              level: 'warning' as const,
              rule: 'company-prefix',
              message: `Plugin name "${String(plugin.name)}" should start with "@mycompany/"`,
            },
          ];
        }
        return [];
