...
import { mergeA11yProps, getInputControlA11yProps } from '@lytjs/common-a11y';
...
export const Table = defineComponent({
  ...
  setup(props: Record<string, unknown>) {
    ...
    return () => {
          ...
          ...(p.showSelection
            ? [
                createVNode('th', { class: 'lyt-table__cell--selection' }, [
                  createVNode(
                    'input',
                    mergeA11yProps(getInputControlA11yProps({ ariaLabel: '全选' }), {
                      type: 'checkbox',
                      checked: allSelected(),
                      onChange: (e: Event) =>
      ...
      const tbody = createVNode(
        'tbody',
        { class: 'lyt-table__body' },
        data.length === 0
          ? [
          ...
          : data.map((row: TableRowData, index: number) => {
              ...
              if (p.showSelection) {
                rowChildren.push(
                  createVNode('td', { class: 'lyt-table__cell--selection' }, [
                    createVNode(
                      'input',
                      mergeA11yProps(getInputControlA11yProps({ ariaLabel: '选择行' }), {
                        type: 'checkbox',
                        checked: selectedRows().has(row),
                        onChange: (e: Event) =>
                          handleRowSelect(row, (e.target as HTMLInputElement).checked),
                      }),
                    ),
                  ]),
                );
              ...
              return createVNode(
                'tr',
                {
                  ...
                  class: ['lyt-table__row', currentRow() === row ? 'lyt-table__row--active' : '']
                    .filter(Boolean)
                    .join(' '),
