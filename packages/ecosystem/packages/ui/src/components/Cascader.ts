  ...
  setup(props: Record<string, unknown>, { slots }: { slots: CascaderSlots }) {
    ...
    const renderOption = (option: CascaderOption, level: number): VNode => {
      ...
      const isSelected = selectedValues().some(
        (path) => path.join('-') === [...activePath().slice(0, level), option.value].join('-'),
      );
      ...
      if (option.loading) {
        optionChildren.push(
          createVNode('span', { class: 'lyt-cascader__loading' }, [createVNode('span', {}, '⏳')]),
        );
      }
      ...
      if (slots.default) {
        ...
        optionChildren.push(
          createVNode('span', mergeA11yProps(labelBtnProps, {}), String(option.label)),
        );
      }

      if (option.children && !option.isLeaf) {
        optionChildren.push(
          createVNode('span', { class: 'lyt-cascader__arrow' }, [createVNode('span', {}, '▶')]),
        );
      }
    ...
    return () => {
      ...
      if (p.multiple && isValueSet) {
        selectedValues().forEach((path, index) => {
          ...
          contentChildren.push(
            createVNode('span', { class: 'lyt-cascader__tag', key: index }, [
              createVNode('span', {}, getPathLabel(path)),
              createVNode(
                'span',
                mergeA11yProps(removeBtnProps, {
        ...
        contentChildren.push(
          createVNode(
            'span',
            {
              class: ['lyt-cascader__value', !isValueSet ? 'lyt-cascader__placeholder' : '']
                .filter(Boolean)
                .join(' '),
            },
      ...
      for (let i = 0; i < maxLevel; i++) {
        ...
        if (options.length > 0 || i === 0) {
          ...
          dropdownContent.push(
            createVNode(
              'div',
              mergeA11yProps(panelProps, { class: 'lyt-cascader__panel', key: i }),
      ...
      const resultChildren: VNode[] = [
        createVNode(
          'div',
          mergeA11yProps(a11yProps, { class: 'lyt-cascader__trigger' }),
          contentChildren,
        ),
      ];
      ...
      if (isDropdownOpen()) {
        ...
        resultChildren.push(
          createVNode('div', { class: 'lyt-cascader__dropdown' }, dropdownChildren),
        );
      }
