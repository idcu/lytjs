...
(['includes', 'indexOf', 'lastIndexOf'] as const).forEach((method) => {
  ...
  arrayInstrumentations[method] = function (this: unknown[], ...args: unknown[]) {
    ...
    if (result === -1 || result === false) {
      return originMethod.apply(arr, args.map(a => toRaw(a)));
    }
    return result;
  };
});
...
function createReactiveObject(
  ...
  const handlers = isMap(target) || isSet(target)
    ? collectionHandlers
    : baseHandlers;
...
export function shallowReadonly<T extends object>(target: T): Readonly<T> {
  return createReactiveObject(
    target, true, true,
    shallowReadonlyHandlers, shallowCollectionHandlers, shallowReadonlyMap
  );
}
