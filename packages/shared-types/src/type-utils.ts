...
export type PartialExcept<T, K extends keyof T> = Partial<Pick<T, Exclude<keyof T, K>>> & Pick<T, K>;

export type RequiredExcept<T, K extends keyof T> = Required<Pick<T, Exclude<keyof T, K>>> & Partial<Pick<T, K>>;
...
export type ExtractKeysByType<T, ValueType> = { [Key in keyof T]-?: T[Key] extends ValueType ? Key : never }[keyof T];
...
export type Awaited<T> = T extends null | undefined ? T : T extends object & { then(onfulfilled: infer F, ...args: any[]): any } ? F extends (value: infer V, ...args: any[]) => any ? Awaited<V> : never : T;
...
export type DeepPick<T, Paths extends string> = Paths extends `${infer Key}.${infer Rest}` ? Key extends keyof T ? { [K in Key]: DeepPick<T[K], Rest> } : never : Paths extends keyof T ? { [K in Paths]: T[K] } : never;
