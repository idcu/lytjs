...
export class PluginValidator {
    ...
    static validateManifest(manifest) {
        ...
        if (manifest.name && !PluginValidator.validateName(manifest.name)) {
            errors.push(`插件名称 "${manifest.name}" 不符合命名规范，应为 lyt-plugin-xxx 或 @scope/lyt-plugin-xxx`);
        }
        // 检查版本号格式
        ...
        if (manifest.category && !VALID_CATEGORIES.includes(manifest.category)) {
            errors.push(`无效的分类 "${manifest.category}"，合法值: ${VALID_CATEGORIES.join(', ')}`);
        }
        // 检查 keywords
    ...
    static validateConfig(config, schema) {
        ...
        for (const [key, value] of Object.entries(config)) {
            ...
            if (expectedType && !PluginValidator.checkType(value, expectedType)) {
                errors.push(`配置项 "${key}" 类型错误：期望 ${expectedType}，实际为 ${typeof value}`);
            }
            // 检查枚举值
            const enumValues = propSchema.enum;
            if (enumValues && Array.isArray(enumValues) && !enumValues.includes(value)) {
                errors.push(`配置项 "${key}" 的值 "${value}" 不在允许范围内: ${enumValues.join(', ')}`);
            }
        }
        return { valid: errors.length === 0, errors, warnings };
    }
     ...
     * @returns 验证结果
     */
    static validateCompatibility(plugin, frameworkVersion) {
        const errors = [];
        const warnings = [];
        // 检查框架版本号格式
        ...
        if (plugin.lytVersion) {
            ...
            if (!satisfied) {
                errors.push(`插件要求框架版本 ${plugin.lytVersion}，当前版本为 ${frameworkVersion}`);
            }
        }
        ...
        if (plugin.peerDependencies && plugin.peerDependencies['@lytjs/core']) {
            ...
            if (!satisfied) {
                errors.push(`插件要求 @lytjs/core 版本 ${range}，当前版本为 ${frameworkVersion}`);
            }
        }
