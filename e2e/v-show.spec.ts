...
test.describe('v-show 显示/隐藏', () => {
  ...
  test('v-show 多个元素独立控制', async ({ page }) => {
    ...
    // 初始状态：A 可见，B 隐藏，C 可见
    let displayA = await page.evaluate(
      () => window.getComputedStyle(document.querySelector('#panel-a')).display,
    );
    let displayB = await page.evaluate(
      () => window.getComputedStyle(document.querySelector('#panel-b')).display,
    );
    let displayC = await page.evaluate(
      () => window.getComputedStyle(document.querySelector('#panel-c')).display,
    );
    ...
    displayA = await page.evaluate(
      () => window.getComputedStyle(document.querySelector('#panel-a')).display,
    );
    displayB = await page.evaluate(
      () => window.getComputedStyle(document.querySelector('#panel-b')).display,
    );
    displayC = await page.evaluate(
      () => window.getComputedStyle(document.querySelector('#panel-c')).display,
    );
