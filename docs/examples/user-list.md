...
function render() {
  return h('div', { id: 'app' }, [
    h('nav', { class: 'navbar' }, [
      h('div', { class: 'nav-brand' }, [h(RouterLink, { to: '/' }, 'User Manager')]),
      h('div', { class: 'nav-links' }, [
        h(RouterLink, { to: '/' }, '用户列表'),
        h(RouterLink, { to: '/about' }, '关于'),
      ]),
    ]),
    h('main', { class: 'main-content' }, [h(RouterView)]),
  ]);
}
...
export async function fetchUsers(page = 1, limit = 10) {
  const response = await fetch(`${API_BASE}/users?_page=${page}&_limit=${limit}`);
...
export function UserCard(props) {
  ...
  return () =>
    h('div', { class: 'user-card', onClick: goToDetail }, [
        ...
        h('div', { class: 'user-list' }, [
          ...
          hasMore.value &&
            ...
            h('div', { class: 'load-more' }, [
              h(
                'button',
                { onClick: loadMore, disabled: loading.value },
            ...
            h('section', { class: 'info-section' }, [
              ...
              h('div', { class: 'info-grid' }, [
                ...
                h('div', { class: 'info-item' }, [
                  ...
                  h(
                    'a',
                    { href: `https://${currentUser.value.website}`, target: '_blank' },
              ...
              h('div', { class: 'info-grid' }, [
                ...
                h('div', { class: 'info-item' }, [
                  ...
                  h(
                    'span',
                    { class: 'catch-phrase' },
                    `"${currentUser.value.company?.catchPhrase}"`,
                  ),
                ]),
                h('div', { class: 'info-item' }, [
                  h('label', {}, '业务'),
