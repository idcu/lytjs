...
- [v6.9.0](./archive/v6.9.0.md) - 编译时优化、性能基准测试、项目优化检查
...
| **测试覆盖率** | 核心包 > 95%（**当前实测全仓 48.8%**，见下方说明） | vitest coverage |
| **内存占用**   | 比 v6.0 降低 40%                                   | 基准测试        |
| **编译速度**   | 比 v6.0 提升 50%                                   | 基准测试        |
| **首屏渲染**   | 减少 30%+                                          | 性能基准测试    |
| **缓存命中率** | > 80%                                              | 缓存统计        |

> ⚠️ **覆盖率实测校正（2026-09-21）**：`vitest.config.ts` 的 `coverage.include`
> 此前只覆盖 `packages/*/src` 与 `packages/common/packages/*/src`，
> 约 49.5% 的源码（ecosystem / plugins / tools）不计入分母，所以"85% 达标"
> 只在半壁江山上成立。分母补全后的真实数字：
>
> | 维度               | 实测       | 目标   |
> | ------------------ | ---------- | ------ |
> | statements / lines | **48.77%** | 85%    |
> | functions          | **63.46%** | 85%    |
> | branches           | **80.09%** | 80% ✅ |
>
> 阈值已按实测下调为 48 / 63 / 80 / 48，并确立**棘轮规则：只允许上调**。
