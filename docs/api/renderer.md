...
| 条件渲染 | 插槽内 `v-if` 编译为三元 `(cond?vnode:null)`（数据源是 transform 产物 `JS_CONDITIONAL_EXPRESSION → VNODE_CALL`）。采用**严格模式**：只要有一段无法完整还原就**整体跳过**，绝不下发半成品                                                                                                                                                                                                           |
| 已知限制 | 插槽内 `v-else` / `v-else-if` / `v-for` / `v-show` 暂不支持；具名插槽（`<template #name>`）暂不支持                                                                                                                                                                                                                                                                                                |
