...
另：核心七个包（compiler / renderer / vdom / component / dom-runtime / core / core-signal）回归 **96 文件 / 2251 用例全绿**。
...
| 项                                 | 说明                                                                                                                                                                                  |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **SSR/客户端一致性：两类已知差异** | ① 表达式类绑定（`:class="[...]"` 等）Signal 编译报错、SSR 可编译；② **组件结构不同**（SSR 直接替换内容、客户端保留 `lyt-comp` 占位容器）⇒ hydration 必然 mismatch，需统一策略         |
| **Signal 表达式支持范围**          | 现仅支持简单属性路径；**方案已就绪**（`docs/design/signal-expression-support.md`，含 20 处替换点、locals 贯穿方案、已验证「简单路径产物不变」）；过渡期已在编译错误里直接给出替代写法 |
| SSR/客户端一致性用例               | **已建**（`ssr-client-consistency.test.ts`，覆盖两端均支持的 5 个场景）                                                                                                               |
